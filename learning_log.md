...
### 3.0. CROSS-LANGUAGE TYPE SYNC (가장 위험한 함정 — 2026-05-24 두 번 당함)

> **Pydantic은 backend의 source of truth, TypeScript interface는 frontend의 same shape의 mirror.**
> **두 언어가 자동 동기화 되지 않으니 사람이 의식적으로 맞춰야 한다.**
> *이게 2026-05-24 우리가 같은 데이터에 대해 두 번 빠뜨린 버그의 본질이다.*

DB schema나 SQL 함수 결과에 필드 하나 추가할 때, **세 layer 모두 같은 PR 안에서 동시 업데이트**해야 함:

1. **SQL function / DB schema** — `RETURN jsonb_build_object(..., 'new_field': value)`
2. **Backend Pydantic `response_model`** — `class XSummary(BaseModel): ...; new_field: Type = default`
3. **Frontend TypeScript interface(s)** — 모든 inline interface + hook의 export type

**빠뜨리면 어떻게 깨지나 (위험도 순):**

| Layer 누락 | 증상 | 발견 난이도 |
|---|---|---|
| **Pydantic `response_model`** | **FastAPI가 silently drop. 에러 없음. 그저 응답 키에서 사라짐.** | **매우 어려움 — quiet bug, 유저 보고로 발견** |
| Frontend TS interface | Vercel TS strict 빌드 fail. CI에서 즉시 잡힘 | 쉬움 (loud, 5분 fix) |

→ **Pydantic 누락이 훨씬 위험.** Loud한 TS error는 push 직후 발견되지만 silent drop은 prod에서 사용자가 "안 보임" 보고할 때까지 모른다.

**왜 FastAPI는 silently drop?** `response_model=X`는 FastAPI *권장 패턴*이고 본래 용도가 있다:
- **보안/privacy** — DB internal field (`password_hash`, `internal_flags`) 자동 제거
- **API 계약 명시** — OpenAPI/Swagger schema 자동 생성, 클라이언트 기대값 문서화
- **응답 안정성** — DB schema 변경이 API 응답을 자동으로 깨뜨리지 않음 (의도된 isolation)

좋은 패턴이지만 **양날의 검**: 의도된 새 필드를 *추가*할 땐 Pydantic도 같이 업데이트 안 하면 quiet 버그. 우리 프로젝트만 그런 것 아니고 FastAPI codebase 어디서나 발생 가능 — 곳곳에 `response_model=` 있는 건 *하드코딩이 아니라 관례*.

**우리 케이스 (오늘 같은 클래스 두 번):**
- PR #14 fixup → `pricing/page.tsx`의 inline TS `UserFeatures` interface가 새 `*_source` 필드 모름 → Vercel build fail (loud, 5분 fix)
- PR #17 → backend `FeaturesSummary` Pydantic class가 같은 필드 모름 → FastAPI가 응답에서 silently drop → 사용자가 "Free trial 라벨 안 보임" 직접 보고할 때까지 1+시간 모름 (quiet, 훨씬 비싸다)

**예방 룰 (가장 강력):**

```bash
# SQL 함수/DB 변경 PR 만들 때 반드시 grep으로 의존하는 모든 layer 찾기
grep -rn "<함수명>\|<column명>" backend/app/api/ frontend/src/
# → 모든 응답/사용처를 같은 PR 안에서 한 번에 update
```

**PR self-check (큰 PR 만들 때):**
- [ ] SQL function output 변경했나? → Pydantic `response_model` 업데이트했나? → 모든 TS interface(특히 inline)도?
- [ ] DB schema 변경했나? → 같은 체크
- [ ] 새 필드면 default 값 정해서 backwards compat 깨지지 않게?

### 3.1. Type duplication (cross-language sync의 frontend-내부 version)

같은 타입을 frontend 안에서도 inline 여러 곳에 두면 새 필드 추가 시 일부만 업데이트 → Vercel/CI 빌드 깨짐. **룰 3.0의 special case** — 같은 본질, 다른 layer.
...
**원칙:** hook의 exported type을 import. 또는 monorepo의 shared types 폴더. inline duplication 절대 안 함.
...
### 4.6. 응답에 필드가 안 보이면 3 layer 모두 점검 (silently drop 의심)

frontend에서 expected 필드가 안 보일 때 추측 순서:
1. **Backend Pydantic `response_model`** ← 가장 흔한 원인 (룰 3.0)
2. SQL 함수가 그 필드 반환하는지 (SQL Editor에서 직접 호출)
3. Frontend TS가 그 필드 destructure하는지
4. 캐시 (브라우저 / Vercel CDN)

→ 우리 케이스 PR #17: SQL Editor에서 `get_user_features_summary` 직접 호출 → 필드 정상 → API 응답엔 없음 → Pydantic 필터 확정.
...
- ❌ **SQL/DB 변경 시 Pydantic `response_model` 업데이트 빠뜨림 (silently drop, 가장 위험한 quiet 버그)** ← 룰 3.0
...
- ❌ SQL 함수만 보고 API 응답 확인 안 하기 — 둘은 다른 layer
...
- ❌ "이 부분 안 건드릴 거야" 가정 — backend Pydantic이 안 보이는 데서 응답 필터 중일 수 있음
...
- **PR #14** — "Owned" vs "Free on first profile" 배지 분기 + double-submit guard + 2nd-profile paywall toast (+ fixup: TS inline interface)
- **PR #15** — Tab-switch latency cascade fix + qa-pairs 무한 로딩 fix + Free trial 라벨링
- **PR #16** — docs: learning_log.md (이 파일)
- **PR #17** — Pydantic `FeaturesSummary` 누락 필드 hotfix — **cross-language type sync 케이스 (룰 3.0)**
