...
### 3.0. CROSS-LANGUAGE TYPE SYNC (가장 위험한 함정)
...
| Layer 누락 | 증상 | 발견 난이도 |
|---|---|---|
| **Pydantic `response_model`** | **FastAPI가 silently drop. 에러 없음.** | **매우 어려움 — quiet bug, 유저 보고로 발견** |
...
**왜 FastAPI는 silently drop?** `response_model=X`는 FastAPI 권장 패턴이고 본래 용도가 있다: 보안/privacy (DB internal field 자동 제거) + API 계약 명시 (OpenAPI 자동 생성) + 응답 안정성 (DB schema 변경이 API 자동으로 안 깨뜨림). 양날의 검 — 의도된 새 필드 추가 시 Pydantic도 같이 안 하면 quiet 버그.

**우리 케이스 (같은 클래스 두 번):**
- PR #14 fixup → `pricing/page.tsx` inline TS `UserFeatures` interface가 새 `*_source` 필드 모름 → Vercel build fail (loud, 5분 fix)
- PR #17 → backend `FeaturesSummary` Pydantic이 같은 필드 모름 → FastAPI가 응답에서 silently drop → 사용자가 "라벨 안 보임" 보고할 때까지 1+시간 모름 (quiet, 훨씬 비싸다)

**예방:**

```bash
# SQL 함수/DB 변경 PR 만들 때 반드시 grep으로 의존 layer 모두 찾기
grep -rn "<함수명>\|<column명>" backend/app/api/ frontend/src/
```

PR self-check: SQL output 변경했나? → Pydantic `response_model` 업데이트? → 모든 TS interface(inline 포함)도?
...
### 6.6. 🔒 유저 답신/PII 익명화 의무 (doc / commit / PR / 대화 어디서나)

이탈 인터뷰 답신, 버그 리포트, 슬랙 캡처 등 유저가 보낸 자료를 doc·commit message·PR body·심지어 대화에 인용할 때 **반드시 익명화**. 한 번 git history에 들어가면 force push로도 완전히 못 지움 (cache, fork, 다른 사람 local clone).
...
|---|---|---|
| 이름 | 맨 앞 한 글자 + `****` | `전****` |
| 이메일 | 맨 앞 한 글자 + `****` + 도메인 유지 | `j****@gmail.com` |
| 답신 본문의 식별 가능 정보 (회사명, 학교명, 사람 이름) | 첫 글자 + `****` | "G****에 다니는" |
| 가입일 | **그대로** (집계 신호로 가치 있고 단독 식별 불가) | 2026-05-16 |
| 위치 (도시·국가) | **그대로** (마케팅·세그먼트 신호) | Lumberton, NC |

**적용 범위:**
- `diary.md`, `learning_log.md`, `plan.md` 등 모든 문서
- git commit message (history는 force push 가능한 시점에만 정정 가능 — 머지 후엔 영구)
- PR title / body / inline comment
- 이 대화 자체 (한 번 친 글자도 사람 화면에 남음 — 의식)

**우리 케이스 (2026-05-25):** 첫 `diary_v2.md` 작성 시 답신자 실명을 4곳에 적었음. 사용자가 즉시 catch. 다행히 PR 만들기 전이라 force push로 git history 정리 가능했지만, *대화에는 그대로 노출됨*. 한 번 친 PII는 *완전 회수 불가*.

**원칙:** 글 쓰기 *전*에 익명화 결정. 쓰고 나서 지우는 건 불완전.

---

## 7. Anti-patterns (절대 안 함)

- ❌ **유저 답신/PII를 익명화 없이 doc·commit·PR·대화에 인용** ← 룰 6.6. 한 번 적힌 PII는 git/cache/사람 기억에서 완전 회수 불가
